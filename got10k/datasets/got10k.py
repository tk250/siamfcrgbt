...
class GOT10k(object):
    ...
    def __init__(self, root_dir, subset='train_i', return_meta=False, 
        ...
        if subset == 'train_i' and visible == False:   
            self.seq_dirs = [os.path.join(root_dir, subset, s, 'infrared')
                             for s in self.seq_names]
            self.anno_files = [os.path.join(root_dir, subset, s, 'infrared.txt')
                             for s in self.seq_names]
        #rgb images
        elif subset == 'train_i' and visible == True:   
            self.seq_dirs = [os.path.join(root_dir, subset, s, 'visible')
                             for s in self.seq_names]
            self.anno_files = [os.path.join(root_dir, subset, s, 'visible.txt')
                             for s in self.seq_names]
        else:
